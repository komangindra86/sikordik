...
7. Sebelum upload bisnis diaktifkan, pasang ClamAV dan qpdf; scan harus berhasil sebelum file dapat diunduh. Terapkan batas upload web server/PHP dan aplikasi secara konsisten. Integrasi scanner, otorisasi dan versioning tersedia sejak Fase 2, tetapi service eksternal harus dikonfigurasi operator.
...
Status terkini: implementasi sampai Fase 8 tersedia, tetapi belum ada izin meluncurkan sistem ke produksi atau menggunakan data pribadi nyata. Lihat [UAT Fase 8](UAT-FASE-8.md) untuk checklist dan latihan backup/restore, serta `sikordik:readiness` untuk pemeriksaan konfigurasi.
